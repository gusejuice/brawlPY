...
class Player:
    ...
    def getName(self): # returns a string of the nickname that the player is using
        return self.__json["name"]
    
    def getTrophies(self): # returns an integer of the current amount of trophies
        return self.__json["trophies"]
    
    def getExpLevel(self): # returns an integer of the current level
        return self.__json["expLevel"]
    
    def getExpPoints(self): 
        return self.__json["expPoints"]
    
    def getHighestTrophies(self): # returns an integer the highest trophy count
        return self.__json["highestTrophies"]
    
    def isQualifiedFromChampionshipChallenge(self): # returns a boolean 
        return self.__json["isQualifiedFromChampionshipChallenge"]
    
    def getThreeVictories(self): # returns an integer of current total of 3v3 victories
        return self.__json["3vs3Victories"]

    def getSoloVictories(self): # returns an integer of current total of solo showdown victories
        return self.__json["soloVictories"]
    
    def getDuoVictories(self): # returns an integer of current total of duo showdown victories
        return self.__json["duoVictories"]
    
    def getBestRoboRumbleTime(self): 
        return self.__json["bestRoboRumbleTime"]
    
    def getBestTimeAsBigBrawler(self):
        return self.__json["bestTimeAsBigBrawler"]

    def getNameColor(self): # returns a color code of the users name color
        try:
